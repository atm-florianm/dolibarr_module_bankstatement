<form method="post" enctype="multipart/form-data" name="bankimport">
	<table class="border" width="100%">
		<tr>
			...
			<td><?php echo $form->select_comptes( ($transactionCompare->account) ? $transactionCompare->account->id : -1,'accountid',0,'courant <> 2',1) ?></td>
			<td width="200"><label for="ds"><?php echo $langs->trans("DateStart") ?></label></td>
			<td><?php echo $form->select_date($transactionCompare->dateStart, 'ds') ?></td>
			<td><label class="fieldrequired" for="numreleve"><?php echo $langs->trans("AccountStatement") ?></label></td>
			<td><input type="text" id="numreleve" name="numreleve" value="<?php echo $transactionCompare->numReleve ?>" /></td>
		</tr>
		<tr>
			...
			<td><?php echo $form->select_date($transactionCompare->dateEnd, 'de') ?></td>
