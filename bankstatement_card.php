...
$permissiontodelete = $user->rights->bankstatement->bankstatement->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	...
	*/
	...
	if ($action != 'presend' && $action != 'editline') {
    	...
    	if (empty($reshook))
    	{
    		...
    		if ($permissiontoadd)
    		{
    			print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&socid='.$object->socid.'&action=clone&object=bankstatement">'.$langs->trans("ToClone").'</a>'."\n";
    		}
